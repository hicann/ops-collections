...
template <typename Key>
void SetupMerge(int filterSizeMiB, std::uint64_t numInputs)
{
    ...
    bloom_filter_perf::FillFilter(*context.destination, 0x5au, context.stream);
    bloom_filter_perf::FillFilter(*context.source, 0xa5u, context.stream);
    context.destination->MergeAsync(*context.source, context.stream);
    Sync(context.stream);
}
...
REGISTER_PERFORMANCE_TEST(bloomFilterMergeI32, (TestMerge<std::uint32_t>), (SetupMerge<std::uint32_t>), int,
                          std::uint64_t);
REGISTER_PERFORMANCE_TEST(bloomFilterMergeI64, (TestMerge<std::uint64_t>), (SetupMerge<std::uint64_t>), int,
                          std::uint64_t);

REGISTER_PERFORMANCE_ARGS(bloomFilterMergeI32, "BloomFilter Merge I32 H1V8 (FilterSizeMiB, NumInput)",
                          ...
                          int, std::uint64_t);
REGISTER_PERFORMANCE_ARGS(bloomFilterMergeI64, "BloomFilter Merge I64 H1V8 (FilterSizeMiB, NumInput)",
                          (std::initializer_list<std::tuple<int, std::uint64_t>>{
                              {32, bloom_filter_perf::NUM_INPUTS},
                              {256, bloom_filter_perf::NUM_INPUTS},
                              {2048, bloom_filter_perf::NUM_INPUTS}}),
