...
public:
    ...
    TestResult() = default;
    TestResult(double cpu, double device, std::size_t iter)
        : cpuTimeUs(cpu), deviceTimeUs(device), iterations(iter) {}

    void Print() const {
        std::cout << "CPU: " << cpuTimeUs 
                  << " us, GPU: " << deviceTimeUs 
                  << " us, Iterations: " << iterations << std::endl;
    }
};
...
public:
    explicit PerformanceTestsRegister(PerformanceTestsFunc func)
        : func_(func), setup_func_(nullptr) {}

    void SetSetup(PerformanceTestsSetupFunc<Args...> setup) {
        setup_func_ = setup;
    }

    void RegisterArgs(const std::string& argsName,
                      std::initializer_list<std::tuple<Args...>> args) {
...
private:
    // 执行测试迭代并收集时间数据
    void RunTestIterations(std::vector<double>& cpuTimes, std::vector<double>& deviceTimes,
                           std::string& stopReason) {
        ...
        for (int iter = 0; iter < max_iters; ++iter) {
            ...
            if (ShouldStopIteration(deviceTimes, totalDeviceTimeS, totalCpuTimeS, minDeviceTimeS, maxCpuTimeS, stopReason)) {
                break;
            }
        }
    }
    ...
    double CalculateStdDev(const std::vector<double>& values, double mean) const {
        if (values.empty()) return 0.0;
        double sqSum = 0.0;
        for (auto t : values) sqSum += (t - mean) * (t - mean);
        return std::sqrt(sqSum / values.size());
    }

    // 计算测试统计数据
    TestStatistics CalculateStatistics(const std::vector<double>& cpuTimes,
                                        const std::vector<double>& deviceTimes,
    ...
    // 输出测试结果
    template <size_t... II>
    void PrintTestResult(const std::string& argsName, const std::tuple<Args...>& sublist,
                         std::index_sequence<II...>, const TestStatistics& stats) {
        std::cout << "========================================" << std::endl;
        ...
        std::cout << "Iterations: " << stats.iterations 
                  << " | Stop Reason: " << stats.stopReason << std::endl;
        std::cout << "Mean CPU Time: " << std::fixed << std::setprecision(0) << stats.meanCpuTime << " us" << std::endl;
        std::cout << "Mean Device Time: " << std::fixed << std::setprecision(0) << stats.meanDeviceTime 
                  << " us (±" << std::fixed << std::setprecision(0) << stats.stddevDeviceTime << " us)" << std::endl;
        std::cout << "========================================" << std::endl;
    }
...
public:
    template<typename... Args>
    PerformanceTestsRegisterManager(const std::string& name,
                                    PerformanceTestsFunc func,
                                    PerformanceTestsSetupFunc<Args...> setup = nullptr) {
        auto registerPtr = std::make_shared<PerformanceTestsRegister<Args...>>(func);
        if (setup) registerPtr->SetSetup(setup);
    ...
    static void SetSetup(const std::string& name, PerformanceTestsSetupFunc<Args...> setup) {
        auto* c = GetCase(name);
        if (c) static_cast<PerformanceTestsRegister<Args...>*>(c)->SetSetup(setup);
    }
...
static struct ArgsRegister_##TEST_NAME { \
    ArgsRegister_##TEST_NAME() { \
        ...
        if (c) static_cast<PerformanceTestsRegister<__VA_ARGS__>*>(c)->RegisterArgs(ARGS_NAME, VALUES); \
    } \
} argsRegister_##TEST_NAME
