...
template <typename K, typename V, int BucketSize>
CreateTestContext<K, V, BucketSize>& GetContext() {
    static CreateTestContext<K, V, BucketSize> ctx;
    return ctx;
}
...
TestResult TestCreate() {
    ...
    auto map = map_factory::MakeStaticMap<Key, Value, BS, map_factory::LinearProbing<Key>>(ctx.capacity, sent, ctx.stream);
...
REGISTER_PERFORMANCE_TEST(createCase1, (TestCreate<uint32_t,uint32_t,1>), (SetupCreateTest<uint32_t,uint32_t,1>), int);
REGISTER_PERFORMANCE_TEST(createCase2, (TestCreate<uint64_t,uint64_t,1>), (SetupCreateTest<uint64_t,uint64_t,1>), int);
