...
TEMPLATE_TEST_CASE("roaring_bitmap destroy test", "[roaring_bitmap][destroy]", uint32_t, uint64_t)
{
    ...
    PRINT_BEFORE_EXEC("destroy test", Key, Key, 1, keys.size(), 0);
    ...
    bitmap.reset();
    REQUIRE_PRINT(!bitmap.has_value());
}
