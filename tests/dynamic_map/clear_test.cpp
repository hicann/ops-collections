...
TEMPLATE_TEST_CASE_SIG("dynamic_map clear correctness", "[dynamic_map][clear]",
                       ...
                       (float, float, 5, aclco::test::dmap_factory::LinearProbing<float>))
{
    ...
    PRINT_BEFORE_EXEC_WITH_PROBE("clear correctness", Key, Value, BS, initCap, n, "", Probe);

    SECTION("clear empties container, then reuse")
    {
        PRINT_SECTION("clear empties container, then reuse");
        ...
        map.Insert(static_cast<void*>(dPairs.Data()), aclco::Extent<std::size_t>(m), stream);
        aclco::test::Sync(stream);
        ...
        std::size_t stillPresent = 0;
        for (auto f : flags)
            if (f != 0) ++stillPresent;
        INFO("still present after clear = " << stillPresent);
        REQUIRE_PRINT(stillPresent == 0);
